...
for i=1:size(s1_matrix,2)
    ...
    v2_substrates_products(rxn_count,1).p2 = prod_2;
...
all_reactions(2,1).product_names = {'C'; 'D'};



% V3
v3=[];
s1_matrix = get_isotopomer_matrix(2);
s2_matrix = get_isotopomer_matrix(1);
...
for i = 1:size(s1_matrix,2)
    for j = 1:size(s2_matrix,2)
        ...
        prod_1 = [curr_s1(1);curr_s1(2);curr_s2(1)];
       
        
        v3_substrates_products(rxn_count,1).s1 = curr_s1;
        v3_substrates_products(rxn_count,1).s2 = curr_s2;
...
all_reactions(3,1).substrate_names = {'C','D'};
all_reactions(3,1).product_names = {'E'};
...
all_reactions(4,1).product_names = {'E'};
...
all_reactions(5,1).substrate_names = {'E'};
all_reactions(5,1).product_names = {'F'};
...
D_matrix = get_isotopomer_matrix(1);
D_chars = get_isotopomer_chars(D_matrix, 'D');

E_matrix = get_isotopomer_matrix(3);
...
for i=1:length(metabolite_vec)
    ...
    if contains(curr_met,'B')==1 || contains(curr_met,'C') ==1 || contains(curr_met,'E')==1
